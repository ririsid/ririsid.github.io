...
- 스위프트의 _String_ 타입은 유니코드 스칼라 값으로 만들어졌다.
...
- 문자열의 _Character_ 값의 수를 얻기 위해 문자열의 _characters_ 프로퍼티의 _count_ 프로퍼티를 사용한다.
...
- 문자열의 각각의 문자에 접근하기 위한 모든 인덱스의 _Range_ 를 만들기 위해 전역 함수 *indices(_:)* 를 사용한다.
